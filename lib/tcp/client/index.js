...
function Client(options) {
    ...
    this.responseTimeout = parseInt(options.responseTimeout) || tcpConsts.DEFAULT_TIMEOUT;
    ...
    this.b = Binary()
        ...
        .tap(function (header) {
            ...
            if (header.func & consts.EXCEPTION_BIT) {
                ...
                if (functionParser) {
                    functionParser.call(that, this, that.currentRequest.type, function(err, values) {
...
Client.prototype._onClose = function() {
    ...
    if (this.currentRequest) {
        ...
        this.emit('close', new errors.ConnectionError('Connection closed'));
    }
    this.socket.destroy(); //?
    this.socket = null; //?
    this._connect();
...
Client.prototype._onError = function(err) {
    ...
    if (this.currentRequest) {
        ...
        this.emit('error', new errors.ConnectionError('Connection Error'));
    }
}
...
Client.prototype._nextRequest = function() {
    if (this._connected && this.queue.length > 0) {
...
Client.prototype._request = function(options) {
    ...
    //default to 16bit
    options.type = options.type||consts.Types.INT16;
    ...
    debug('Got pdu', pdu, (pdu)?pdu.length: 0);
    ...
    this.timeout = setTimeout(this._onTimeout.bind(this), this.responseTimeout);
